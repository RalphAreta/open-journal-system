...
<form method="POST" action="{{ route('submissions.update', $submission) }}" enctype="multipart/form-data" class="max-w-2xl space-y-4">
    ...
    <div>
        ...
        <textarea id="abstract" name="abstract" rows="5" required
            class="mt-1 block w-full rounded-md border-slate-300 shadow-sm focus:border-red-500 focus:ring-red-500 @error('abstract') border-red-500 @enderror">{{ old('abstract', $submission->abstract) }}</textarea>
