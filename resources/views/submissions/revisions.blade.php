    ...
    <div class="space-y-6">
        @foreach ($submission->revisionRequests->sortByDesc('requested_at') as $revision)
            @php
                ...
                $role = null;
                if ($requester) {
                    if ($requester->hasRole('editor-in-chief')) $role = ['label' => 'Editor-in-Chief', 'class' => 'bg-purple-100 text-purple-700'];
                    elseif ($requester->hasRole('editor'))       $role = ['label' => 'Editor',          'class' => 'bg-blue-100 text-blue-700'];
                    elseif ($requester->hasRole('reviewer'))     $role = ['label' => 'Reviewer',        'class' => 'bg-teal-100 text-teal-700'];
                }
            @endphp

            <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-8
                {{ $revision->isResolved() ? 'opacity-75' : '' }}">

                <div class="flex items-start justify-between mb-6">
                    <div>
                        <h2 class="text-2xl font-bold text-slate-900 mb-2">
                            {{ ucfirst($revision->revision_type) }} Revision Requested
                        </h2>
                        <div class="flex items-center gap-2 flex-wrap">
                            <p class="text-sm text-slate-600">
                                Requested by <span class="font-semibold text-slate-900">{{ $requester?->name ?? 'Unknown' }}</span>
                            </p>
                            @if ($role)
                                <span class="inline-block px-2 py-0.5 rounded-full text-xs font-semibold {{ $role['class'] }}">
                                    {{ $role['label'] }}
                                </span>
                            @endif
                ...
                @if ($revision->isResolved())
                    <div class="bg-green-50 border border-green-200 rounded-lg p-4">
                        <p class="text-green-700 font-semibold">✓ Revised Manuscript Submitted</p>
                        <p class="text-sm text-green-600 mt-1">
                            Submitted on {{ $revision->revised_at->format('F d, Y \a\t h:i A') }}
                        </p>
                        @if ($revision->revision_notes)
                            <p class="text-sm text-slate-600 mt-3">
                                <span class="font-semibold">Your Notes:</span> {{ $revision->revision_notes }}
                            </p>
                        @endif
                    </div>
                @else
                    <form method="POST" action="{{ route('submissions.submit-revision', $submission) }}"
                        ...
                        <div>
                            <label class="block text-sm font-semibold text-slate-900 mb-2">
                                Upload Revised Manuscript <span class="text-red-600">*</span>
                            </label>
                            <div class="border-2 border-dashed border-slate-300 rounded-lg p-6 text-center hover:border-red-500 transition-colors cursor-pointer"
                                 id="file-drop-{{ $revision->id }}">
                                <input type="file" name="file" required accept=".pdf,.doc,.docx"
                                       class="hidden" id="file-input-{{ $revision->id }}">
                        ...
                        <div>
                            <label class="block text-sm font-semibold text-slate-900 mb-2">
                                Revision Notes <span class="text-red-600">*</span>
                            </label>
                            <textarea name="revision_notes" required rows="4"
                                class="w-full px-4 py-3 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-red-500"
                                placeholder="Describe the changes you made in this revision..."></textarea>
                        ...
                        <button type="submit"
                            ...
                            ✓ Submit Revised Manuscript to {{ $requester?->name ?? 'Requester' }}
                        </button>
                    </form>
                @endif
            </div>
