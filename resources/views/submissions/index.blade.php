...
<div class="flex justify-between items-center mb-6">
    ...
    <a href="{{ route('submissions.create') }}" class="bg-red-600 text-white px-4 py-2 rounded-md hover:bg-red-700 font-medium shadow-sm transition-colors">New Submission</a>
</div>
    ...
    <table class="min-w-full divide-y divide-slate-200">
        <thead class="bg-slate-50">
            <tr>
                ...
                <th class="px-4 py-3 text-left text-xs font-medium text-slate-700 uppercase">Submitted</th>
        ...
        <tbody class="divide-y divide-slate-200">
            @forelse($submissions as $s)
                <tr>
                    <td class="px-4 py-3 text-sm text-slate-900">{{ Str::limit($s->title, 55) }}</td>
                    <td class="px-4 py-3"><span class="px-2 py-1 text-xs rounded-full bg-slate-100 text-slate-900">{{ $s->status }}</span></td>
                    <td class="px-4 py-3 text-sm text-slate-700">{{ $s->submitted_at?->format('M d, Y') ?? '-' }}</td>
                    <td class="px-4 py-3 text-right">
                        ...
                        @if($s->isEditableByAuthor())
                            <a href="{{ route('submissions.edit', $s) }}" class="ml-2 text-red-600 hover:text-red-700 hover:underline text-sm font-medium">Edit</a>
                        @endif
                    </td>
                </tr>
            @empty
                <tr><td colspan="4" class="px-4 py-8 text-center text-slate-700">No submissions. <a href="{{ route('submissions.create') }}" class="text-red-600 hover:text-red-700 font-medium">Create one</a>.</td></tr>
            @endforelse
        </tbody>
    </table>
