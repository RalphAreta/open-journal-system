...
@section('content')
    <div class="font-body max-w-6xl mx-auto space-y-6">
        ...
        >
            <div>
                ...
                <p class="text-sm text-slate-500 mt-1">
                    Manage and monitor your research paperss
                </p>
            </div>
            <div class="flex items-center gap-3 self-start md:self-auto">
                ...
                <a
                    ...
                    class="inline-flex items-center gap-1.5 bg-red-600 hover:bg-red-700 text-white px-5 py-2.5 rounded-[9px] text-[11px] font-bold uppercase tracking-[.07em] transition-all hover:-translate-y-0.5 shadow-md shadow-red-100/80 hover:shadow-lg hover:shadow-red-200/50 whitespace-nowrap"
                >
                    + New Submission
                </a>
            </div>
        ...
        {{-- ── Search Bar ── --}}
        <div
            class="bg-white border border-slate-200 rounded-[14px] p-3.5 shadow-sm flex flex-col md:flex-row gap-3 items-center fade-up-1"
        >
            <div class="relative flex-1 w-full">
                ...
                <input
                    ...
                    class="w-full pl-10 pr-4 py-2.5 bg-slate-50 border border-slate-200 rounded-[9px] text-sm font-body text-slate-700 placeholder:text-slate-400 focus:outline-none focus:border-red-400 focus:bg-white focus:ring-2 focus:ring-red-500/10 transition-all"
                    onkeyup="filterBoard()"
                />
            </div>
            <div
                class="hidden md:flex items-center gap-2 px-3 py-2 bg-slate-50 border border-slate-100 rounded-[9px] text-[10px] font-bold uppercase tracking-[.07em] text-slate-400 whitespace-nowrap"
            >
                <span
        ...
        {{-- ── Table Card ── --}}
        <div
            class="bg-white border border-slate-200 rounded-[14px] overflow-hidden shadow-sm fade-up-2"
        >
            <table class="w-full text-left" id="boardTable">
                <thead class="bg-slate-50 border-b border-slate-200">
                    <tr>
                        <th
                            class="px-6 py-3.5 text-[10px] font-bold uppercase tracking-[.09em] text-slate-400"
                        >
                            Manuscript Title
                        </th>
                        <th
                ...
                <tbody>
                    @forelse ($submissions as $s)
                        <tr
                            class="board-row border-b border-slate-100 last:border-0 hover:bg-slate-50/60 transition-colors group"
                        >
                            <td class="px-6 py-4">
                                <p
                                    class="text-sm font-semibold text-slate-900 group-hover:text-red-600 transition-colors title-cell leading-snug"
                                >
                                    {{ Str::limit($s->title, 75) }}
                                </p>
                                <p
                                    class="text-[11px] text-slate-400 font-mono mt-1"
                                >
                            ...
                            <td class="px-6 py-4">
                                ...
                                <span
                                    class="status-cell inline-flex items-center gap-1.5 px-3 py-1 rounded-full border text-[10px] font-bold uppercase tracking-[.04em] {{ $cls }}"
                                >
                                    <span
                            ...
                            <td class="px-6 py-4">
                                <span
                                    class="date-cell text-xs font-medium text-slate-500"
                                >
                            ...
                            <td class="px-6 py-4 text-right">
                                ...
                                >
                                    ...
                                    @if ($s->isEditableByAuthor())
                                        <span
                                            class="text-slate-200 select-none"
                                        >
                                            |
                                        </span>
                                        <a
    ...
    <script>
        function filterBoard() {
            ...
            document.querySelectorAll('.board-row').forEach((row) => {
                row.style.display = row.innerText.toUpperCase().includes(filter)
                    ? ''
                    : 'none';
            });
        }
