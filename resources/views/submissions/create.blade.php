...
<form method="POST" action="{{ route('submissions.store') }}" enctype="multipart/form-data" class="max-w-2xl space-y-4">
    ...
    <div>
        ...
        <input id="title" type="text" name="title" value="{{ old('title') }}" required
            class="mt-1 block w-full rounded-md border-slate-300 shadow-sm focus:border-red-500 focus:ring-red-500 @error('title') border-red-500 @enderror">
    ...
    <div>
        ...
        <textarea id="abstract" name="abstract" rows="5" required
            class="mt-1 block w-full rounded-md border-slate-300 shadow-sm focus:border-red-500 focus:ring-red-500 @error('abstract') border-red-500 @enderror">{{ old('abstract') }}</textarea>
    ...
    <div class="flex gap-2">
        <button type="submit" class="bg-red-600 text-white px-4 py-2 rounded-md hover:bg-red-700 font-medium shadow-sm transition-colors">Submit</button>
