...
<ul class="list-disc list-inside text-slate-600">
    @forelse($role->users as $u)
        <li><a href="{{ route('admin.users.edit', $u) }}" class="text-indigo-600 hover:underline">{{ $u->name }}</a> ({{ $u->email }})</li>
    @empty
        <li>No users</li>
    @endforelse
</ul>
<p class="mt-4"><a href="{{ route('admin.roles.index') }}" class="text-indigo-600 hover:underline">← Back to roles</a></p>
@endsection
