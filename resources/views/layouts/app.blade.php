...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        <style>
            ...
            .mobile-menu[data-open="true"]  { display: block; animation: slideDown .22s ease forwards; }
            ...
            input:focus, textarea:focus, select:focus {
                ...
                box-shadow: 0 0 0 3px rgba(45,129,118,.1), 0 4px 12px rgba(45,129,118,.15) !important;
            ...
            .btn-primary {
                transition: all 0.28s cubic-bezier(0.34, 1.56, 0.64, 1);
                box-shadow: 0 4px 14px rgba(45,129,118,.25);
            ...
            .btn-primary:hover {
                ...
                box-shadow: 0 12px 28px rgba(45,129,118,.35);
            }

            /* ── Badges ── */
            .badge { transition: all .25s ease; display: inline-flex; align-items: center; gap: .5rem; }
            .badge-success { background: linear-gradient(135deg,#f0fdf4,#ecfdf5) !important; border: 1.5px solid #86efac !important; color: #166534 !important; box-shadow: 0 2px 8px rgba(34,197,94,.12); }
            .badge-danger  { background: linear-gradient(135deg,#fef2f2,#fef1f1) !important; border: 1.5px solid #fca5a5 !important; color: #991b1b !important; box-shadow: 0 2px 8px rgba(239,68,68,.12); }
            .badge-warning { background: linear-gradient(135deg,#fffbeb,#fef3c7) !important; border: 1.5px solid #fce181 !important; color: #b45309 !important; box-shadow: 0 2px 8px rgba(217,119,6,.12); }
            .badge-info    { background: linear-gradient(135deg,#f0f9ff,#e0f2fe) !important; border: 1.5px solid #7dd3fc !important; color: #0e7490 !important; box-shadow: 0 2px 8px rgba(6,182,212,.12); }
            .badge:hover { transform: translateY(-2px); }

            /* ── Cards ── */
            .card {
                transition: all .32s cubic-bezier(0.34,1.56,0.64,1);
                border-color: #e8dfd0 !important; border-width: 1.5px;
            }
            .card:hover {
                transform: translateY(-6px);
                box-shadow: 0 20px 48px rgba(45,129,118,.12) !important;
                border-color: rgba(45,129,118,.3) !important;
            }

            /* ── Alerts ── */
            .alert { border-radius: 12px; border-width: 1.5px; box-shadow: 0 4px 12px rgba(0,0,0,.08); transition: all .25s ease; }
            .alert-success { background: linear-gradient(135deg,#f0fdf4,#ecfdf5); border-color: #86efac; color: #166534; }
            .alert-danger  { background: linear-gradient(135deg,#fef2f2,#fef1f1); border-color: #fca5a5; color: #991b1b; }
            .alert-warning { background: linear-gradient(135deg,#fffbeb,#fef3c7); border-color: #fce181; color: #b45309; }

            /* ── Table rows ── */
            .table tbody tr { transition: all .2s ease; border-bottom-color: #e8dfd0 !important; }
            .table tbody tr:hover { background-color: #f9f6f0 !important; box-shadow: inset 4px 0 0 0 #2d8176; }

            /* ── Spinner ── */
            .spinner-pulse { animation: pulse-subtle 2s ease-in-out infinite; }
    ...
    <body class="min-h-screen bg-[#f5f0e8] text-[#0d1628] font-source antialiased flex flex-col">
        ...
        {{-- ═══════════════════════════════════════
             NAVIGATION
        ═══════════════════════════════════════ --}}
        <nav class="bg-[#2D8176] shadow-xl sticky top-0.75 z-50 border-b border-white/10"
            ...
            <div class="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8">
                <div class="flex items-center justify-between h-16 md:h-20">
                    ...
                    <div class="flex items-center gap-3 md:gap-6 min-w-0">
                        ...
                        <a href="{{ url('/') }}"
                           class="flex items-center space-x-2.5 group transition-all shrink-0">
                            <div class="relative w-9 h-9 md:w-12 md:h-12 flex items-center justify-center
                                        bg-gradient-to-br from-[#c9a84c] to-[#a07830]
                                        rounded-full border-2 border-white/30 shadow-inner
                                        group-hover:rotate-12 transition-transform duration-500">
                                <svg class="text-white w-5 h-5 md:w-7 md:h-7"
                            ...
                            <div class="flex flex-col leading-none">
                                <span class="font-libre text-base sm:text-xl md:text-2xl font-bold text-white tracking-tight">
                                    Journal System
                                </span>
                                <span class="hidden sm:block text-[8px] md:text-[9px] font-bold text-[#f0d678]
                                             tracking-[0.2em] uppercase opacity-90 mt-0.5">
                        ...
                        @auth
                            ...
                            <div class="hidden md:flex items-center space-x-1 ml-4 border-l border-white/10 pl-4">
                                ...
                                @if ($activeRole === 'editor-in-chief')
                                    @if (Route::has('appeals.index'))
                                        <a href="{{ route('appeals.index') }}" class="{{ $linkBase . $inactive }}">APPEALS</a>
                                    @endif
                                @endif
                                @if ($activeRole === 'admin')
                    ...
                    <div class="flex items-center gap-1.5 sm:gap-3">
                        ...
                        @auth
                            @php
                                $activeRole = session('active_role') ?? optional(auth()->user()->roles->first())->name ?? 'author';
                                ...
                                $unreadNotifs = $notifQuery->latest()->take(5)->get();
                            @endphp

                            {{-- Desktop: user name + role switcher --}}
                            <div class="hidden lg:flex flex-col items-end mr-1 text-right">
                                <span class="text-[9px] font-bold text-[#f0d678] uppercase tracking-widest leading-none mb-1">
                                    Authenticated
                                </span>
                                <span class="text-sm font-medium text-white/95 truncate max-w-[9rem]">
                                    {{ auth()->user()->name }}
                                </span>
                            </div>
                            ...
                            @if (count($userRoles) > 1)
                                <div class="relative hidden md:block group">
                                    <button class="px-3 py-1.5 bg-white/10 text-white text-[11px] font-bold
                                                   tracking-widest rounded-lg hover:bg-white/20 transition-all
                                                   uppercase border border-white/20 whitespace-nowrap">
                                        {{ ucfirst(str_replace('-', ' ', $activeRole)) }} ▼
                                    </button>
                                    <div class="absolute right-0 mt-2 w-48 bg-[#1a4d46] border border-white/20
                                                rounded-lg shadow-xl opacity-0 invisible
                                                group-hover:opacity-100 group-hover:visible transition-all z-50">
                                        @foreach ($userRoles as $role)
                                            <a href="{{ route('dashboard.switch-role', $role) }}"
                                               class="block px-4 py-2.5 text-white text-sm
                                                      {{ session('active_role') === $role ? 'bg-white/20 font-bold text-[#f0d678]' : 'hover:bg-white/10' }}
                            ...
                            <div class="relative" id="notification-bell-wrapper"
                                ...
                                <div id="notification-dropdown"
                                     ...
                                     class="absolute right-0 mt-2 w-80 bg-white border border-[#ede8e0]
                                            rounded-2xl shadow-2xl z-50 overflow-hidden"
                                     style="display:none">

                                    <div class="px-4 py-3 border-b border-[#ede8e0] bg-[#faf8f5]
                                                flex items-center justify-between">
                                        <p class="text-[10px] font-black uppercase tracking-widest text-[#2D8176]">
                                            Notifications
                                        </p>
                                    </div>
                                    ...
                                    <div id="notification-list" class="max-h-[60vh] overflow-y-auto">
                                        @forelse ($unreadNotifs as $notif)
                                            <div class="px-4 py-3 border-b border-[#f0ece6] hover:bg-[#faf8f5]
                                                        transition-colors bg-blue-50/50 notification-item">
                                                <div class="flex items-start gap-3">
                                                    <div class="shrink-0 mt-1.5">
                                                        ...
                                                        @elseif ($notif->type === 'warning')
                                                            <span class="w-2 h-2 rounded-full bg-amber-500 block"></span>
                                                        @else
                                                            <span class="w-2 h-2 rounded-full bg-blue-500 block"></span>
                                                        @endif
                                                    </div>
                                                    <div class="flex-1 min-w-0">
                                                        <p class="text-[12px] font-bold text-[#0d1628] leading-snug">
                                                            {{ $notif->title }}
                                                        </p>
                                                        <p class="text-[11px] text-[#6a7890] mt-0.5 leading-relaxed">
                                                            {{ Str::limit($notif->message, 80) }}
                                                        </p>
                                                        <p class="text-[9px] text-[#b0aaa0] mt-1">
                                        ...
                                        @empty
                                            <div class="px-4 py-8 text-center">
                                                <svg class="w-8 h-8 text-[#d0ccc5] mx-auto mb-2" fill="none"
                                                     stroke="currentColor" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"
                                                          d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6 6 0 10-12 0v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9"/>
                                                </svg>
                                                <p class="text-[12px] text-[#b0aaa0]">No new notifications</p>
                                            </div>
                                        @endforelse
                                    </div>

                                    <div class="px-4 py-2.5 bg-[#faf8f5] text-center border-t border-[#ede8e0]">
                                        <a href="{{ route('notifications.index') }}"
                                           class="text-[10px] font-black uppercase tracking-widest
                                                  text-[#2D8176] hover:text-[#1f5d54] transition-colors">
                                            View All →
                                        </a>
                                    </div>
                                </div>
                                ...
                                <script>
                                    function markRead() {
                                        ...
                                        .then(data => {
                                            ...
                                            if (badge && data.unread_count === 0) badge.remove();
                                            document.querySelectorAll('.notification-item')
                                                    .forEach(i => i.classList.remove('bg-blue-50/50'));
                                        })
                                        .catch(console.error);
                                    }
                            ...
                            <form method="POST" action="{{ route('logout') }}" id="logout-form">
                                ...
                                <button type="button" onclick="confirmLogout()"
                                        class="hidden sm:inline-flex items-center px-4 py-1.5
                                               bg-gradient-to-br from-[#c9a84c] to-[#a07830]
                                               text-white text-[11px] font-bold tracking-widest
                                               rounded-full shadow-lg hover:shadow-[#a07830]/40
                                               transition-all active:scale-95 uppercase border border-white/20">
                                    Logout
                                </button>
                                {{-- Mobile icon --}}
                                <button type="button" onclick="confirmLogout()"
                                        class="sm:hidden flex items-center justify-center w-9 h-9
                                               bg-white/10 hover:bg-white/20 rounded-lg
                                               text-white/80 hover:text-white transition-all border border-white/15"
                                        aria-label="Logout">
                            ...
                            <button id="hamburger-btn"
                                    class="md:hidden flex flex-col items-center justify-center gap-[5px]
                                           w-9 h-9 rounded-lg bg-white/10 hover:bg-white/20
                                           transition-all border border-white/15 ml-0.5"
                        ...
                        @else
                            ...
                            {{-- Home: always visible on desktop, hidden on mobile (in drawer) --}}
                            <a href="{{ url('/') }}"
                               class="hidden sm:block text-sm font-bold text-white/90
                                      hover:text-[#f0d678] transition-colors uppercase tracking-wider mr-1">
                                Home
                            </a>

                            @if (! Route::is('login'))
                                <a href="{{ route('login') }}"
                                   class="hidden sm:inline-flex px-4 sm:px-6 py-2 sm:py-2.5
                                          bg-gradient-to-br from-[#c9a84c] to-[#a07830]
                                          text-white text-xs sm:text-sm font-bold tracking-wide
                                          rounded-xl shadow-lg hover:-translate-y-0.5 transition-all
                                          active:translate-y-0 border border-white/10">
                                    LOGIN →
                                </a>
                            @endif
                            ...
                            @if (! Route::is('register'))
                                <a href="{{ route('register') }}"
                                   class="hidden sm:inline-flex px-6 py-2.5
                                          bg-gradient-to-br from-[#c9a84c] to-[#a07830]
                            ...
                            <button id="guest-hamburger-btn"
                                    class="sm:hidden flex flex-col items-center justify-center gap-[5px]
                                           w-9 h-9 rounded-lg bg-white/10 hover:bg-white/20
                ...
                <div id="guest-mobile-menu"
                    ...
                    <div class="max-w-7xl mx-auto px-4 py-4 space-y-1.5">
                        ...
                        @if (! Route::is('register'))
                            <a href="{{ route('register') }}"
                               class="flex items-center gap-3 px-4 py-3.5 rounded-xl
                                      text-sm font-semibold text-[#f0d678] hover:text-white
                                      bg-white/8 hover:bg-white/15 transition-all border border-[#c9a84c]/30"
                               onclick="toggleGuestMenu()">
                                <svg class="w-4 h-4 shrink-0" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round"
            ...
            @auth
                <div id="mobile-menu"
                    ...
                    <div class="max-w-7xl mx-auto px-4 py-4 space-y-1">
                        ...
                        {{-- User info strip --}}
                        <div class="flex items-center gap-3 px-3 py-3 mb-3
                                    bg-white/10 rounded-xl border border-white/10">
                            <div class="w-9 h-9 rounded-full bg-gradient-to-br from-[#c9a84c] to-[#a07830]
                            ...
                            <div class="min-w-0">
                                ...
                                <p class="text-[#f0d678] text-[10px] uppercase tracking-widest font-bold">
                        ...
                        <a href="{{ route('dashboard') }}" class="{{ $mLink }}">
                            <svg class="w-4 h-4 shrink-0" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                        ...
                        @if ($activeRole === 'author')
                            <a href="{{ route('submissions.index') }}" class="{{ $mLink }}">
                                <svg class="w-4 h-4 shrink-0" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                        ...
                        @endif
                        @if ($activeRole === 'reviewer')
                            <a href="{{ route('reviews.index') }}" class="{{ $mLink }}">
                                <svg class="w-4 h-4 shrink-0" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                        ...
                        @endif
                        @if ($activeRole === 'editor')
                            <a href="{{ route('editor.submissions') }}" class="{{ $mLink }}">
                                <svg class="w-4 h-4 shrink-0" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                        ...
                        @if ($activeRole === 'editor-in-chief' && Route::has('appeals.index'))
                            <a href="{{ route('appeals.index') }}" class="{{ $mLink }}">
                                <svg class="w-4 h-4 shrink-0" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                        ...
                        @endif
                        @if ($activeRole === 'admin')
                            <a href="{{ route('admin.users.index') }}" class="{{ $mLink }}">
                                <svg class="w-4 h-4 shrink-0" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                        ...
                        @if (count($userRoles) > 1)
                            <div class="pt-3 mt-3 border-t border-white/10">
                                <p class="px-3 mb-2 text-[9px] font-black uppercase tracking-[.2em] text-[#f0d678]">
                                    Switch Role
                                </p>
                                <div class="grid grid-cols-2 gap-1.5">
                                    @foreach ($userRoles as $role)
                                        <a href="{{ route('dashboard.switch-role', $role) }}"
                                           class="px-3 py-2.5 rounded-xl text-center text-xs font-semibold
                        ...
                        <div class="pt-3 mt-2 border-t border-white/10">
                            <button type="button" onclick="confirmLogout()"
                                    class="w-full flex items-center gap-3 px-4 py-3 rounded-xl
                                           text-sm font-semibold text-red-300 hover:text-red-200
        ...
        {{-- ══════════════════════════════════════
             MAIN CONTENT
        ══════════════════════════════════════ --}}
        <main class="grow max-w-7xl mx-auto w-full
                     py-6 md:py-12
            ...
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 md:py-12">
                <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-8 md:gap-12">
                    ...
                    <div class="sm:col-span-2">
                        <h3 class="font-libre text-xl md:text-2xl font-bold text-white mb-3 md:mb-4">
                            Journal System
                        </h3>
                    ...
                    <div>
                        <h4 class="text-[11px] font-bold text-[#f0d678] uppercase tracking-[0.2em] mb-4 md:mb-6">
                            Portal Links
                        </h4>
        ...
        <script>
            ...
            const guestHamburger = document.getElementById('guest-hamburger-btn');
