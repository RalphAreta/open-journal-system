...
<form method="POST" action="{{ route('reviews.store') }}" class="max-w-2xl space-y-4">
    ...
    <div>
        ...
        <textarea id="comments_for_author" name="comments_for_author" rows="4"
            class="mt-1 block w-full rounded-md border-slate-300 shadow-sm focus:border-red-500 focus:ring-red-500">{{ old('comments_for_author') }}</textarea>
    </div>
    <div>
        ...
        <textarea id="comments_for_editor" name="comments_for_editor" rows="4"
            class="mt-1 block w-full rounded-md border-slate-300 shadow-sm focus:border-red-500 focus:ring-red-500">{{ old('comments_for_editor') }}</textarea>
    </div>
    <div>
        ...
        <input id="rating" type="number" name="rating" min="1" max="5" value="{{ old('rating') }}"
            class="mt-1 block w-20 rounded-md border-slate-300 shadow-sm focus:border-red-500 focus:ring-red-500">
    </div>
    <div class="flex gap-2">
        <button type="submit" class="bg-red-600 text-white px-4 py-2 rounded-md hover:bg-red-700 font-medium shadow-sm transition-colors">Submit Review</button>
