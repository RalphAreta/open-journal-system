...
@push('styles')
    <style>
        /* ══ MOBILE RESPONSIVE for Revision Review ══ */
        @media (max-width: 640px) {
            ...
            /* Submission info grid: 1 column */
            .rev-info-grid {
                grid-template-columns: 1fr !important;
                gap: 12px !important;
            }

            /* Files grid: already grid-cols-1 md:grid-cols-2, fine */

            /* Recommendation radio: 1 column on very small, 2 col on ≥360px */
            .rev-rec-grid {
                grid-template-columns: 1fr !important;
                gap: 10px !important;
            }
            ...
            /* Rating scale table: already overflow-x-auto, just tighter font */
            .rev-scale-table td,
            .rev-scale-table th {
        ...
        @media (min-width: 360px) and (max-width: 640px) {
            /* 2-col radio grid once we have a little room */
            .rev-rec-grid {
...
@section('content')
    <div class="rev-wrap max-w-4xl mx-auto py-8 px-4">
        <div class="mb-8">
            <nav
            ...
            <h1
                class="rev-main-title text-4xl font-black text-slate-900 tracking-tighter mb-2"
            >
        ...
        {{-- Submission Info --}}
        <div
            class="rev-card bg-white rounded-lg shadow border border-slate-200 p-6 mb-6"
        >
            <div class="rev-info-grid grid grid-cols-2 gap-4 mb-4">
                <div>
                    <p
        ...
        @if ($revisionReview->revisionRequest->revision_notes)
            <div
                class="rev-card bg-blue-50 border border-blue-200 rounded-lg p-6 mb-6"
            >
                <p
                    class="text-xs font-bold text-blue-700 uppercase tracking-widest mb-2"
                >
                    Author's Revision Notes
                </p>
                <p class="text-slate-700 whitespace-pre-wrap text-sm">
                    {{ $revisionReview->revisionRequest->revision_notes }}
                </p>
            </div>
        @endif
        ...
        <div
            class="rev-card bg-slate-50 border border-slate-200 rounded-lg p-6 mb-6"
        >
            <p
                class="text-xs font-bold text-slate-600 uppercase tracking-widest mb-2"
            >
                Original Revision Request
            </p>
            <div class="flex items-center justify-between mb-3 flex-wrap gap-2">
                <span
                    class="px-3 py-1 rounded-full text-xs font-semibold {{ $revisionReview->revisionRequest->revision_type === 'minor' ? 'bg-yellow-100 text-yellow-700' : 'bg-orange-100 text-orange-700' }}"
                >
                    {{ $revisionReview->revisionRequest->revision_type === 'minor' ? '⚡ Minor Revisions' : '🔴 Major Revisions' }}
                </span>
                <span class="text-xs text-slate-500">
                    {{ $revisionReview->revisionRequest->requested_at->format('M d, Y') }}
                </span>
            </div>
            <p class="text-sm text-slate-700 whitespace-pre-wrap mb-4">
                {{ $revisionReview->revisionRequest->reason }}
            </p>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            ...
            <div
                class="rev-card bg-white rounded-lg shadow border border-slate-200 p-6"
            >
                <div
                ...
                @if ($currentRevision->revised_file_path)
                    <div>
                        <p class="text-sm font-medium text-slate-700 mb-1">
                            {{ $currentRevision->revised_file_name }}
                        </p>
                        <p class="text-xs text-slate-500">
                            Revised on
                            {{ $currentRevision->revised_at->format('M d, Y') }}
                        </p>
                    </div>
                    <a
                        href="{{ route('submissions.revision-file.download', ['submission' => $submission, 'revisionRequest' => $currentRevision]) }}"
                ...
                >
                    ...
                    @if ($previousRevision->revised_file_path)
                        <div>
                            ...
                            <p class="text-xs text-slate-500">
                                Revised on
                                {{ $previousRevision->revised_at->format('M d, Y') }}
                            </p>
                        </div>
                        <a
        ...
        <div
            class="rev-card bg-white rounded-lg shadow border border-slate-200 p-6"
        >
            <h2 class="rev-form-title text-2xl font-bold text-slate-900 mb-6">
                Submit Your Revision Review
            </h2>

            <form
                method="POST"
                action="{{ route('reviews.revision-store') }}"
                class="space-y-6"
            >
                @csrf
                <input
                    type="hidden"
                    name="revision_review_id"
                    value="{{ $revisionReview->id }}"
                />
                ...
                <div>
                    ...
                    <div class="rev-rec-grid grid grid-cols-2 gap-3">
                        @foreach (\App\Models\RevisionReview::recommendationOptions() as $value => $label)
                            <label
                                class="flex items-center p-4 border-2 border-slate-200 rounded-lg cursor-pointer hover:border-red-300 transition-colors {{ old('recommendation', $revisionReview->recommendation) === $value ? 'border-red-600 bg-red-50' : '' }}"
                            >
                                <input
                                    type="radio"
                                    name="recommendation"
                                    ...
                                    {{ old('recommendation', $revisionReview->recommendation) === $value ? 'checked' : '' }}
                                />
                                <span
                ...
                <div>
                    ...
                    >
                        ...
                        <span class="text-slate-500 font-normal text-xs">
                            (These will be visible to author)
                        </span>
                    </label>
                    <textarea
                    ...
                    >
{{ old('comments_for_author', $revisionReview->comments_for_author) }}</textarea
                    >
                ...
                <div>
                    ...
                    >
                        Comments for Editor
                        <span class="text-slate-500 font-normal text-xs">
                            (Confidential - not shared with author)
                        </span>
                    </label>
                    <textarea
                    ...
                    >
{{ old('comments_for_editor', $revisionReview->comments_for_editor) }}</textarea
                    >
                ...
                <div class="bg-slate-50 border border-slate-200 rounded-lg p-6">
                    ...
                    <div class="overflow-x-auto -mx-2 px-2">
                        <table
                            class="rev-scale-table w-full text-xs min-w-[420px]"
                        >
                            <thead>
                            ...
                            <tbody>
                                ...
                                >
                                    ...
                                    <td class="p-2 text-slate-700">
                                        Significant gaps in addressing feedback
                                    </td>
                                </tr>
                                <tr
                                ...
                                >
                                    ...
                                    <td class="p-2 text-slate-700">
                                        Multiple issues inadequately addressed
                                    </td>
                                </tr>
                                <tr
                                ...
                                >
                                    ...
                                    <td class="p-2 text-slate-700">
                                        Most feedback addressed; minor gaps
                                    </td>
                                </tr>
                                <tr
                                ...
                                >
                                    ...
                                    <td class="p-2 text-slate-700">
                                        Thorough engagement; significant
                                        improvements
                                    </td>
                                </tr>
                                <tr class="bg-green-50">
                                    ...
                                    <td class="p-2 text-slate-700">
                                        Exceptional responsiveness; extensive
                                        improvements
                                    </td>
                                </tr>
                            </tbody>
                ...
                <div>
                    ...
                    <div class="rev-rating-row flex items-center gap-3">
                        <input
                            id="rating-revision"
                            type="number"
                            name="rating"
                        ...
                        <span
                            id="ratingRevisionInterpretation"
                ...
                {{-- Buttons --}}
                <div
                    class="rev-btn-row flex items-center justify-between pt-6 border-t border-slate-200 gap-3"
                >
                    <a
                        href="{{ route('reviews.index') }}"
                    ...
                    <div class="flex items-center gap-3 flex-wrap justify-end">
                        <button
                            ...
                            name="action"
                            value="save_draft"
                            class="bg-slate-400 hover:bg-slate-500 text-white px-6 py-3 rounded-lg text-sm font-bold uppercase tracking-[.06em] transition-all duration-200 hover:-translate-y-0.5 shadow-md shadow-slate-200/80 hover:shadow-lg hover:shadow-slate-300/50 whitespace-nowrap"
                        >
                            Save &amp; Submit Later
                        </button>
                        <button
                            ...
                            name="action"
                            value="submit"
                            class="bg-slate-900 hover:bg-red-600 text-white px-6 py-3 rounded-lg text-sm font-bold uppercase tracking-[.06em] transition-all duration-200 hover:-translate-y-0.5 shadow-md shadow-slate-200/80 hover:shadow-lg hover:shadow-red-200/50 whitespace-nowrap"
                        >
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            function updateInterpretation(rating) {
                ...
                if (rating >= 1 && rating <= 20)
                    interpretation = 'Critical failures';
                else if (rating >= 21 && rating <= 40)
                    interpretation = 'Inadequate revision';
                else if (rating >= 41 && rating <= 60)
                    interpretation = 'Acceptable responses';
                else if (rating >= 61 && rating <= 80)
                    interpretation = 'Good revision work';
                else if (rating >= 81 && rating <= 100)
                    interpretation = 'Exemplary revision';
            ...
            if (ratingInput) {
                ...
                if (ratingInput.value) updateInterpretation(ratingInput.value);
            }
        });
    </script>
@endsection
